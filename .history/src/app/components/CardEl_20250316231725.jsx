...
const CardEl = () => {
  const [cards, setCards] = useState(
    colors.map((color, index) => ({ id: index.toString(), color, height: Math.floor(Math.random() * 100) + 150 })) // Added random height for masonry effect
  );
  const [activeId, setActiveId] = useState(null);
  const numColumns = 3; // You can adjust the number of columns
